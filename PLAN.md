...
- [x] **F2.1 The model takes a dial.** `Sim.set_reel(r)` in [-1, 1] with `REEL_INERTIA`; the three zones of §4.3d. `test_tuning.gd` first: the crank's settle point scales with `reel`; hold during a run loads the line and give does not; give drops tension faster than hold and costs distance; pressure tires the fish whenever the line is loaded. `set_reeling` is deleted, not wrapped. *Done 2026-09-12. One correction measured on the way: a held line during a run has to carry the fish's whole resistance, not only the pull term, or the middle of the slide is free (NOTES.md)*
- [x] **F2.2 The bots on the dial, and GIVER.** Seven policies, each failing for its own reason; `balance.gd` and `probe_loss.gd` re-measured by band and written into NOTES.md against the fifth fight's numbers; the golden re-recorded and its diff READ - every changed number explained by the model change or the run stops. *Done 2026-09-12 with a new instrument, `scripts/probe_dial.gd`, which is what set the bots: strain recovers slower so the late thumb pays, HUMAN's hands feel the rod late and shove, ANGLER's valve aims just under the red. Human ladder 100 / 100 / 100 / 99 / 76 / 58 against the fifth fight's 100 / 100 / 100 / 99 / 72 / 56*
- [x] **F2.3 The slide.** The Cast button becomes the reel slide in FIGHTING: vertical track, spring return to centre, scaled dead zone, the 1.7 curve, ±280 px. A `test_controls.gd`-style assertion through the REAL handler: a drag up sets `reel` positive and a drag down negative, in proportion; release returns to zero over the spring with no single-frame step. The bot seam drives it: `bot_touch_pixels` returns a point up or down the track and the template driver turns a moved point into a `ScreenDrag` on the same finger instead of a lift (template change, forward-ported). *Done 2026-09-12. Travel is 260 px up and 150 down, not 280 each way: a knob pulled fully down has to clear the gesture bar, and the smoke suite asserts that inequality. `test/test_controls.gd` is the gate (four tests, one the bot's pixel round trip); the knob is the reel handle and turns with the rod's crank; the intro and the hint say the slide's words now*
- [x] **F2.4 The pressure gauge up the left side**, and the top-third rule replaced. Smoke asserts: visible only in a fight and comes back after hiding; fills with `sim.tension` (same number that parts the line); the danger mark sits at `DANGER`; it pulses above the line; clear of both thumb zones and the frame's centre; `mouse_filter` ignore. *Done 2026-09-12. Its top is 880 px from the top edge, under the sounder's column, and its bottom 0.68 of the height - pixels above because the sounder is pixels, a fraction below because the thumb zone is; the smoke suite asserts both relations. The pulse reads the heavy haptic's own timer, so eye and palm beat together. `shot.gd -- 0.7 strain` photographs it over the line*
- [x] **F2.5 The distance readout as a measurement.** Digits and a metre scale, quiet, top-centre; the brass case deleted. Smoke asserts the ORDER: gauge area and contrast above the counter's. *Done 2026-09-12: a line counter 56 px tall and half the width, a ruler with a tick per metre, the net's reach in green, a still marker and outlined digits over it. The order is asserted as rect height (a quarter of the gauge's) and ink alpha (scale 0.58, digits 0.80, both under the gauge's case 0.82). The first cut was pale ink with no shadow and vanished into the dawn sky: quiet is not invisible. The fourth fight's needle, written every frame and read by nothing, went with the bar*
- [x] **F2.6 The face of it: knob, crank, morph.** The knob turns with the thumb, the rod's crank spins with `reel` both ways and stalls into a run, the Cast-to-slide cross-fade. `shot.gd` grows `fight` (a fish on, the slide up) so the control can be photographed; filmed with the bot at 1/60 s tiles across a give-and-take. *Done 2026-09-12: filmed at 30 fps through a whole fight and read at 4-frame tiles round the strike, the run and the landing. The knob, the crank and the gauge all answer in the same tile. The film found what no screenshot could: both ends of the cross-fade said the new word at once for six frames. Fixed (a fading control keeps its last word), asserted one frame into each fade, verified by reintroducing the bug. Judgement in NOTES.md*
- [x] **F2.7 Sound and touch.** Reel clicks at the crank's rate; the drag sound under give-and-load; the heavy train's gap from 100 ms to 50 ms with tension. Smoke asserts the gap shrinks monotonically and the drag sound is silent when no line is given. *Done 2026-09-12: four clicks per turn off the crank's own angle (a slow crank ticks, a fast one whirrs; giving line has the drag's ratchet instead), `Audio.drag_level` sings for a running fish and for line given under load and not for slack line, and the heavy train starts at the WARN line at 170 ms and closes to 50 ms at the top, its weight climbing with it - the gauge's pulse reads the same timer, so eye and palm beat together from the same moment. The tap-fired click and its signal went; there is no tap in a fight*
