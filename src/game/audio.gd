...
## THE FOURTH LAYER IS BIRDS, and it is the only one that is a place rather than
## a weather.
##
## W6: "Lakeside, not generic woodland. Fades out entirely by mid-depth, beside
## the bells." It is the morning half of the arc made audible - a dawn chorus off
## the reeds that thins as the sun gets up and is gone entirely once the line is
## down in Old Town. It leaves at the same rate `bed_bells` does, deliberately, so
## the two absences are one absence and the deep is quiet for one reason rather
## than two.
const AMBIENCE := ["amb_water", "amb_wind", "amb_deep", "birds"]
...
func _stream(id: String, loop: bool) -> AudioStream:
	if _cache.has(id):
		return _cache[id]
	# OGG OR WAV. Everything generated by `make_audio.gd` is a WAV, and the two
	# sounds that could not be generated - a real oarlock and a real dawn chorus -
	# are field recordings, which arrive as OGG and would be absurd to store
	# uncompressed. The loader takes either rather than the callers knowing which.
	var path := ""
	for ext in [".ogg", ".wav"]:
		var try_path := "res://assets/audio/%s%s" % [id, ext]
...
func tick(dt: float, in_room: bool) -> void:
	...
	# BIRDS: loudest at first light, thinning through the morning, silent by the
	# afternoon and silent at depth whatever the hour. Multiplied rather than
	# picked, so a dawn cast into the quarry has no birds on it - the water
	# closing over the line is the thing that takes them away.
	var hour_birds := 0.0
	match sim.hour:
		"dawn": hour_birds = 1.0
		"morning": hour_birds = 0.45
		"dusk": hour_birds = 0.18
		_: hour_birds = 0.0
	_amb[3].volume_db = master - 21.0 + _db(
